...
sed -e "s|TRAINING_VM_REPO=.*|TRAINING_VM_REPO=\"$REPO_URL\"|" \
    ...
    cloud-init/provisioning.sh > cloud-init/provisioning.sh.tmp
...
write_files:
  - path: /root/provisioning.sh
    permissions: '0755'
    content: |
$(sed '      s/^/      /' cloud-init/provisioning.sh.tmp)
...
rm "$SEED_ISO" user-data meta-data cloud-init/provisioning.sh.tmp
